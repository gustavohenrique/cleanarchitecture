...
func TestTodoItemGatewayUpdate(ts *testing.T) {
	...
	test.WithSqlite(ts, "Should update fields", func(t *testing.T, store interfaces.ISqlDataStore, ctx context.Context) {
	...
	test.WithSqlite(ts, "Should fail when title is empty", func(t *testing.T, store interfaces.ISqlDataStore, ctx context.Context) {
		assert.Nil(t, store.Exec(insertTodoItemQuery, item.ID, item.Title, item.IsDone))
