...
import (
	...
	"{{ .ProjectName }}/src/infra/datastores/dgraph/customjson"
...
func New(config Config) interfaces.IGraphDataStore {
	return &DgraphStore{
		config: config,
...
func (db *DgraphStore) WithContext(ctx context.Context) interfaces.IGraphDataStore {
	db.ctx = ctx
	return db
}
